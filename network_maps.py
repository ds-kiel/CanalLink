...
def get_color_map(df, name, metric):
    ...
    # Save map to HTML file
    map_file_path = f'./HTML-Maps/get_color_map-{name}-{metric}.html'
    m.save(map_file_path)
    print(f"Map saved to {map_file_path}")
...
def get_map_CircleMarker(df, name, metric):
    ...
    #name='starlink'
    # Save map to HTML file
    map_file_path = f'./HTML-Maps/get_map_CircleMarker-{name}-{metric}.html'
    m.save(map_file_path)
    print(f"Map saved to {map_file_path}")
...
def get_color_based_on_score_adaptive(score, min_val, max_val, cmap_name='RdYlGn'):
    """
    ...
    Parameters:
        ...
        cmap_name (str): Name of the colormap (default: 'RdYlGn').
    ...
    colormap = cm.get_cmap(cmap_name)  # Get the colormap
    rgba_color = colormap(normalized_score)  # Get RGBA color
    hex_color = mcolors.to_hex(rgba_color)  # Convert to hex
    
    return hex_color

def display_regions_on_map_adaptive(df, metric):
    """
    Display regions on a Folium map with color coding based on the given metric.
    
    Parameters:
        df (pd.DataFrame): Dataframe containing lat/lon boundaries and metric values.
        metric (str): The name of the metric column.

    Returns:
        folium.Map: Folium map object.
    """
    # Compute min and max for scaling colors
    min_val, max_val = df[metric].min(), df[metric].max()
    print(f'min_val: {min_val}, max_val: {max_val}')

    # Initialize a Folium map centered at the average coordinates
    lat_center = (df['lat_min'].mean() + df['lat_max'].mean()) / 2.0
    lon_center = (df['lon_min'].mean() + df['lon_max'].mean()) / 2.0
    m = folium.Map(location=[lat_center, lon_center], tiles="Cartodb Positron", zoom_start=11)
    
    # Add rectangles with dynamically scaled colors
    for _, row in df.iterrows():
        bounds = [[row['lat_min'], row['lon_min']], [row['lat_max'], row['lon_max']]]
        color = get_color_based_on_score_adaptive(row[metric], min_val, max_val) 
...
def create_coverage_squares_with_metric(df, identity, data_class, date, metric, lon_step = 0.0008, lat_step = 0.0004):
    ...
    map_regions = display_regions_on_map_adaptive(region_scores, metric)

    # Save to file
    map_regions.save(f'./HTML-Maps/{identity}-{data_class}-{date}-{metric}.html')  # Save the map to an HTML file
